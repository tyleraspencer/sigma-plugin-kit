...
## Data: use the default. Don't ask, don't fake it.

Default source, already wired into `build-plugin-workbook.py`: connection
**Sigma Sample Database**, path
`RETAIL.PLUGS_ELECTRONICS.PLUGS_ELECTRONICS_HANDS_ON_LAB_DATA`, grouped by
`STORE_REGION` with `Sum(PRICE * QUANTITY)` as `Revenue`.

- **Do not ask fake-vs-real.** Build on the default and state which source you
  used. Only go elsewhere if the user names a source.
- **Never generate an input table for synthetic data.** Sigma cannot populate
  one from a spec, so it publishes empty, the plugin falls through to its own
  fallback, and you ship a chart of numbers hardcoded in the HTML that looks
  real. Full evidence in `docs/plugins.md`.

Columns on the default table, and how to discover a different source
(`list-connections.sh`, `mcp-search.sh`, `mcp-describe.sh`): `docs/plugins.md`.
